...
const Board = () => {
  ...
  const { isGameStarted, bombs, boardSizes, cells } = useSelector((state) => state.game)
  ...
  const cellClickCallback = (x, y) => cellClickHandler(x, y, cells, isGameStarted, bombs, lastY, lastX, playCorrect, dispatch)

  const markCallback = (x, y) => markCell(x, y, cells, dispatch)

  const revealAroundCallback = (x, y) => {
    revealAroundCell(x, y, cells, lastX, lastY, playCorrect, playError, dispatch)
  }

  const mouseUpHandler = (event) => {
    if (event.nativeEvent.which === 1 && event.buttons === 0) clickHandler(event, cellClickCallback)
    else if (event.nativeEvent.which === 3) rightClickHandler(event, markCallback)
  }
  ...
  const cellsElements = cells.map((row, y) =>
    row.map((cell, x) => <Cell key={width * y + x} type={cell.type} value={cell.value} coord={{ x, y }} />)
  )

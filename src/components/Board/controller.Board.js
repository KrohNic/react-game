...
import { updateCells, showEndWindow, decreaseBombLeft, increaseBombLeft, setGameStarted } from '../../redux/actions'
...
const getCoordCellsAround = (x, y, lastX, lastY) => {
  ...
  const cellsAround = [top, bottom, left, right, topLeft, topRight, bottomLeft, bottomRight]

  return cellsAround.filter((obj) => obj)
}

export const createCells = (bombs, lastY, lastX) => {
  const cells = new Array(lastY + 1).fill(undefined).map(() => new Array(lastX + 1).fill(undefined).map(() => ({ type: BTN, value: 0 })))
...
const revealCellsArea = (startX, startY, cells, lastX, lastY) => {
  ...
  while (queue.length) {
    ...
    cellsAround.forEach((coord) => {
      if (cells[coord.y][coord.x].type !== REVEAL && cells[coord.y][coord.x].value !== BOMB) {
        queue.push(coord)
      }
    })
  }
...
export const revealAroundCell = (x, y, cells, lastX, lastY, playCorrect, playError, dispatch) => {
  ...
  surroundingCells.forEach((coord) => {
    if (cellsCopy[coord.y][coord.x].type === BTN) revealCell(coord.x, coord.y, cellsCopy, lastX, lastY, playCorrect, dispatch)
  })
...
export const clickHandler = (event, callback) => runCbIfCell(event.target, callback)
export const bothClickHandler = clickHandler
export const rightClickHandler = clickHandler

export const cellClickHandler = (x, y, cellsData, isGameStarted, bombs, lastY, lastX, playCorrect, dispatch) => {
  if (cellsData[y][x].type !== BTN) return

  let cellsDataCopy = [...cellsData]

  if (!isGameStarted) {
    cellsDataCopy = firstClickHandler(x, y, cellsDataCopy, bombs, lastY, lastX, dispatch)
  }
